...
{\small\begin{theorem} Suppose that
	\begin{gather*}
		\cg \text{ is a normed space}\qquad \SOL \text{ is homogeneous} \qquad \Lambda = \text{all homogeneous functionals}\\
	...
	\end{gather*}
Then \alert{no algorithm} exists that can solve $(\SOL, \CRIT, \Lambda)$.
\end{theorem}}
{\small\begin{proof}\squeezeeqspace{2pt}
\only<1>{Suppose such an algorithm exists.  Fix $(\tolabs,\tolrel)$.  Let $f=0$ be the input.  After $n$ function values the algorithm terminates with the data matrix $\begin{pmatrix} \vL_n &\vzero_n \end{pmatrix}$ and numerical solution $\OUT$.  There exists some $h \in \cf$  with $\norm[\cg]{\SOL(h)} > 0$ and $\vL_n(h) = \vzero_n$.  So for any $c > 0$, the input $\pm ch$ causes the algorithm to terminate with the  \alert{same} $\OUT$.  Thus, \ldots \let\qedsymbol\relax}\only<2>{\vspace{-2ex}
\begin{align*}
	\max(\tolabs,\tolrel c\norm[\cg]{\SOL(h)}) &
