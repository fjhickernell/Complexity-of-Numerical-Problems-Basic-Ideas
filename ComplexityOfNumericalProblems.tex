...
\begin{frame}{Trapezpoidal Rule Integration Satisfying a Compound Criterion}
For the integration problem above
\begin{gather*}
	\cf = \{f \in C[0,1] : \norm[2]{f''} \le 1\}, \qquad 
	\cg  = \reals,  \qquad
	...
	\CRIT(\OUT, \SOL(f),(\tolabs, \tolrel))  = \begin{cases} \ttrue, & \abs{\SOL(f) - \OUT} \le \max(\tolabs,\tolrel \abs{\SOL(f)}), \\
		\tfalse, & \text{otherwise}, 
		\end{cases} \\
	 \Lambda  = \{L :  L(f) = f(x) \text{ for some } x \in \Omega\} = \text{function evaluations}\\
\end{gather*}
The following data accumulation and stopping criteria will succeed
\begin{gather*}
\end{gather*}
