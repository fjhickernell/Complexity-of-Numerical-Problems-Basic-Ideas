...
For mathematical problems, $\SOL:\cf \to \cg$, one may prove \alert{existence}
	...
	\intertext{One may construct \alert{analytical solutions} via symbolic computation \cite{Coh03a} for some \alert{smaller $\cf$} than that which admits existence:}
			\multispan2{$\SOL(x \mapsto x \exp(-2x)) = \frac {1 - 3 \exp(-2)}4$} &
			\multispan2{$\SOL(x \mapsto 4 x^2 - 6x + 1)  = \left \{ \frac {3 \pm \sqrt{5}}{4}  \right\}$}
	\intertext{Analytical solutions, $\SOL(f)$ may \alert{not be available} if $f$ is too complex. Then we need \alert{numerical solutions}}
			\multispan2{\hfil$\SOL(\text{black-box}) =?$\hfil} &
            \multispan2{\hfil$\SOL(\text{black-box}) =?$\hfil} 
	\end{align*}
We discover information about input function $\text{black-box}(\cdot)$---apart from its membership in $\cf$---through  \alert {data operators} applied to  $\text{black-box}(\cdot)$.
...
\begin{frame}{Data}
	...
	\vspace{-5ex}
	Now we look at \alert{algorithms} to solve numerical problems.
...
\begin{frame}{Are There Numerical Problems with No Possible Algorithm?}
\squeezeeqspace{6pt}
	\vspace{-2ex}
	\begin{theorem} Suppose that
	\begin{gather*}
		\cg \text{ is a normed space}\qquad \SOL \text{ is homogeneous} \qquad \Lambda = \text{all linear functionals}\\
		\cf = \text{cone of input functions with the property that for all $\vL_n$ there exists an $h \in \cf$ } \\
		 \text{with $\vL_n(h) = \vzero_n$ and $\norm[\cg]{\SOL(h)} > 0$, 
			e.g.,  an $\infty$-dimensional vector space} \\
		\CRIT(\OUT,\SOL,(\tolabs,\tolrel))  = \ttrue \iff  \norm[\cg]{\SOL(f) - \OUT} \le \max(\tolabs,\tolrel \norm[\cg]{\SOL(f)})
	\end{gather*}
Then no algorithm exists that can solve $(\SOL, \CRIT, \Lambda)$.
\end{theorem}
\begin{proof}\squeezeeqspace{2pt}
\only<1>{Suppose such an algorithm exists.  Fix $(\tolabs,\tolrel)$.  Let $f=0$ be the input.  After $n$ function values the algorithm terminates with the data matrix $\begin{pmatrix} \vL_n &\vzero_n \end{pmatrix}$ and numerical solution $\OUT$.  There exists some $h \in \cf$  with $\norm[\cg]{\SOL(h)} > 0$ and $\vL_n(h) = \vzero_n$.  So for any $c > 0$, if $ch$ were the input, the algorithm would terminate with the $\OUT$. Since $\pm ch$ yield the same $\OUT$, \ldots \let\qedsymbol\relax}\only<2>{\vspace{-2ex}
\begin{align*}
	\max(\tolabs,\tolrel c\norm[\cg]{\SOL(h)}) &
...
\end{align*}
which is impossible.  (Remember Goldilocks.)
}
\end{proof}
