...
\vspace{-5ex}
Want numerical solution, $\OUT$, to be close to true solution, $\SOL(f)$, according to a specified error criterion, $\CRIT$, and tolerance, $\veps$:
\begin{align*}
...
\intertext{An \alert{acceptable numerical solution} satisfies $\CRIT(\OUT,\SOL(f),\veps) = \text{true}$. In addition to knowledge about the input function based on the definition of $\cf$, the algorithm constructing the numerical solution samples the input function, e.g., by generating function values, or perhaps derivative values:}
    \Lambda &= \text{set of allowable data functionals on } \cf
\end{align*}
The description of a \alert{numerical problem} can be written as the ordered triple $(\SOL, \CRIT, \Lambda)$
\end{frame}
