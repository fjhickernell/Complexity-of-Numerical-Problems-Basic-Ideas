...
For mathematical problems, $\SOL:\cf \to \cg$, one often seeks to prove \alert{existence}
	...
	\intertext{One may seek \alert{analytical solutions} via symbolic computation \cite{Coh03a} for some \alert{smaller $\cf$} than that which admits existence:}
...
\begin{frame}{Choices of $\cf$}
	\vspace{-5ex}
	The set of input functions, $\cf$, chosen to prove \alert{existence} or to construct \alert{analytical solutions}.  For numerical problems $\cf$ should be chosen
	\begin{itemize}
		\item Often as a \alert{ball} or \alert{cone}, usually not a vector space
