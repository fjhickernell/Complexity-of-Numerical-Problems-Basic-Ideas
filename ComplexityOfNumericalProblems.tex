...
\]
where $0 < n_1 < n_2 < \cdots$, and $n_{\ell+1}$ depends on $\mZ_{n_\ell}$.  $L_1, L_2, \ldots$ is the \alert{design}, which in general may be \alert{adaptive}.
...
\]
determines whether there exists an output, $\OUT$, that satisfies the error criterion, $\CRIT(\OUT,\SOL(f),\veps)$
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\State $\mZ = (\vL,\mY) \leftarrow (\emptyset, \emptyset)$
		
		\State $\text{done} \leftarrow \tfalse$
		
		\Repeat 
		
		\State Construct $\cg(\mZ) \supseteq \{\SOL(f) :   f \in \cf, \ \vL(f) = \mY   \}$ \\ \hfill \hfill $= \text{ set of solutions consistent with function data}$
		
		\If {$\exists \OUT \in \cg \text{ such that }  \CRIT(\OUT,g,\veps) = \ttrue \ \forall g \in \cg(\mZ)$}
		
		\State $\text{done} \leftarrow \ttrue$
		
		\Else 
		
		\State $\mZ = (\vL,\mY) \leftarrow \DATA(f,\mZ)$
		
		\EndIf
		
		\Until{$\text{done} = \ttrue$}
		
		\RETURN $\OUT$, $\COST(f,\veps) \leftarrow \text{\# elements in } \mY$
	\end{algorithmic}
\end{algorithm}	
\end{frame}
