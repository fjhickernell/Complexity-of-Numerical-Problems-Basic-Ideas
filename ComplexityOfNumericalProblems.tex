...
\begin{gather*}
	...
	\CRIT(\OUT, \SOL(f),(\tolabs, \tolrel))  = \begin{cases} \ttrue, & \abs{\SOL(f) - \OUT} \le \max(\tolabs,\tolrel \abs{\SOL(f)}), \\
		\tfalse, & \text{otherwise}, 
		\end{cases} \\
	 (\tolabs,\tolrel) \in (0,\infty) \times [0,1)\\
...
\mZ_{2^{m}+1} =  \begin{pmatrix} \mX_{2^{m}+1} & f(\mX_{2^{m}+1}) \end{pmatrix} = \DATA(f,\veps,\mZ_{2^{m-1}+1}) = \begin{pmatrix} \mX_{2^{m-1}+1} & f(\mX_{2^{m-1}+1}) \\ 1/2^m & f(1/2^m) \\ \vdots & \vdots \\ 1 - 1/2^m & f(1 - 1/2^m)\end{pmatrix} \quad \text{for } m \in \naturals
\end{gather*}
...
\begin{gather*}
	...
	\CRIT(\OUT, \SOL(f),(\tolabs, \tolrel))  = \begin{cases} \ttrue, & \abs{\SOL(f) - \OUT} \le \max(\tolabs,\tolrel \abs{\SOL(f)}), \\
		\tfalse, & \text{otherwise},
		\end{cases} \\
		\text{\alert{trapezoidal rule}: } \TRAP(\mZ_{2^m+1}) = \frac 1{2^m} \left( \frac 12 f(0) + f(1/2^m) + \cdots + f(1 - 2^m) + \frac 12 f(1) \right)
\end{gather*}
