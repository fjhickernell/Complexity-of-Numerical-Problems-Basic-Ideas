...
Algorithms take \alert{time} to complete their work, but this may be machine-dependent.  So we often measure the computational cost of an algorithm in terms of \alert{floating point operations}.  Specifically, we would like to know the \alert{information cost}, denoted $\ICOST$, or \alert{number of function data} required by the algorithm to return the desired result.  We are often satisfied with the order as $\veps \to \vzero$.
...
\[
\ICOST(\ALG) = \Order\Bigl(\bigl[\min(\tolabs, \tolrel \abs{\SOL(f)})\bigr]^{-1/2} \Bigr)
\]
\end{frame}
